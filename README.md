...
Edit `content/chapters/13-introduction-backtest.md`. Source boundaries and verified page locators are in `references/introduction-backtest-sources.md`. This lesson uses only the owner-supplied resources; the supplied PDFs stay local. New charts use deterministic Route 3 rendering, with no generated artwork.

Rebuild and execute the standard-library notebook and synthetic fixtures with `python3 scripts/build_backtest_intro.py`. Run `node --test tests/backtest-intro.test.mjs` to verify timing, exact post-fee rebalancing, costs, liquidation and JS/Python consistency. The fixed seed is 20260925; the sample is 259 open-to-open intervals (open 60 to open 319). These are synthetic teaching results, not market evidence.
